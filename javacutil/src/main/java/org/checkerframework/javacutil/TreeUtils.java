...
public final class TreeUtils {
   ...
   *
   * @param method the {@link MethodTree} for a node that may be an anonymous constructor
   ...
   * implicitly assigned to the record's fields). This may be an explicitly declared canonical
   * constructor or an implicitly generated one.
   *
  ...
  /**
   * Determines whether or not the node referred to by the given {@link Tree} is part of a record
   * that has been automatically generated by the compiler. This can be a field that is derived from
   * the record's header field list, or an automatically generated canonical constructor. The result
   * is undefined if you pass a {@link Tree} that is not the member of a record.
   *
   * @param member the {@link Tree} for a node that may be automatically generated.
   * @return true if the given path is generated by the compiler, false if it does not
   */
  public static boolean isAutoGeneratedRecordMember(final Tree member) {
    @Nullable Element e = elementFromTree(member);
    if (!(e instanceof Symbol)) {
      return false;
    }
    try {
